...
CONNECTED_IFACE=$(ip route get 192.168.99.70 2>/dev/null | grep -oP 'dev \K\S+' || echo "unknown")
...
if ! ping -c 1 -W 2 -I wlp0s13f0u4 192.168.99.70 &>/dev/null; then
    echo "FAIL: Cannot reach iperf3 server at 192.168.99.70"
    exit 1
fi
